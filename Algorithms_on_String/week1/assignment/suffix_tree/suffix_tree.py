...
def build_suffix_tree(text = 'CAGTCAGG'):
	...
	text += '$'
	inf_list = [ [None, None] for _ in range(len(text) + 1)]
	inf_list[iter_] = [0, len(text)]
	tree[iter_] = {text[0] : iter_ + 1}
	iter_ += 1

	for i in range(1, len(text) - 3): # remove - 4
		cur = 0
		count = 0
		...
		if text[i] in tree[cur].keys():
			...
			j = i
			while (j < length) and (text[j] == text[start_idx]):
			...
			if j == length:
				...
				tree[tree[cur][text[i]]] = {text[inf_list[iter_][0] + 1] : iter_ + 1}
				iter_ += 1

				# update inf_list with the new branch
				inf_list[iter_] = [j, length - j]
				# create the new node from the parent node info
