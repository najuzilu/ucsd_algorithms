...
def build_tree(patterns, text):
	tree = dict()
	iter_node = 0

	tree[iter_node] = {patterns[0][0]: [iter_node + 1, 0, len(patterns[0])]}
	iter_node += 1

	for i in range(1, 2):
		pattern = patterns[i]
		current_node = 0
		sample_string_count = 0


		for j in range(len(pattern)): # loop through strings of pattern
			string = pattern[j]
			node = match_from_root(tree, current_node, string, text)
			print('Looping STRING=',string)
			if node: # string iter found on branch
				print(tree[current_node][node])
				sample_string_count += 1
				pass # update current_node idx
			else: # not found in tree[current_node]
				tree[current_node][string] = [iter_node, j, len(pattern) - j]
				break
